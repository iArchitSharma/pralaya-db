...
function createBackup(config, outputFile, callback, backupType) {
  ...
  switch (backupType.toLowerCase()) {
    ...
    case "incremental":
      const incrementalBackupDir = "./backups/";
      const timestamp = Date.now();
      const incrementalFile = `${incrementalBackupDir}/mysql-bin-incremental-${timestamp}.log`;

      backupCommand = spawn("mysqlbinlog", [
    ...
    case "differential":
      const differentialFile = `/backups/differential-${Date.now()}.sql`;

      const differentialQuery = `
            mysqldump -h ${host} -P ${port} -u ${user} --password=${password} --where="last_modified > 'LAST_BACKUP_TIMESTAMP'" ${database} > ${differentialFile}
          `;

      backupCommand = spawn("bash", ["-c", differentialQuery]);
      break;
